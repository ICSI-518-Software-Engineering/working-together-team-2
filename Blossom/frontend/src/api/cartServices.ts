...
const API_URL = 'http://ec2-18-116-231-38.us-east-2.compute.amazonaws.com:8086/api/cart';
...
export const createOrders = async (userId: string, details: CustomerDetails) => {
  try {
    const response = await axios.post('http://ec2-18-116-231-38.us-east-2.compute.amazonaws.com:8086/api/cart/create-orders', { userId, details });
...
export const addToCart = async (params: AddToCartParams): Promise<void> => {
  try {
    const response = await axios.post('http://ec2-18-116-231-38.us-east-2.compute.amazonaws.com:8086/api/cart/update-cart', params);
...
export const buyNow = async (userId: string, productId: string, vendorId: string, details: CustomerDetails) => {
  try {
    const response = await axios.post('http://ec2-18-116-231-38.us-east-2.compute.amazonaws.com:8086/api/order/buy-now', {
      userId,
      productId,
      vendorId,
      details
